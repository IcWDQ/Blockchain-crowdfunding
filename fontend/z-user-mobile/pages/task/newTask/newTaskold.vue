		...
		<view class="exchange_bg">
			<view class="jui_bg_fff jui_lrtop box   ">
				<view class="jui_flex jui_flex_items_center">
					...
					<text class="jui_fc_333 jui_fs32 jui_mar_l20">{{symbol_name}}</text>
				</view>
				<view class=" jui_flex jui_flex_items_center jui_mar_t16">
					<view class="jui_fs26" style="width:16%;">
						策 略
					</view>
					<scroll-view class="scroll-view_H " scroll-x="true">

						<view class="" style="width: 80%">
							<radio-group @change="radioChange" class="jui_flex">
								<label class="uni-list-cell uni-list-cell-pd  scroll-view-item_H"
									v-for="(item, index) in taskDeail" :key="index">
									<view class="jui_flex jui_flex_items_center">
										<view>
										...
										<!-- <view :class="item.type == '限'?'text-line':'text-rad'" class="jui_mar_l12">
											{{item.type}}
				...
				<view class=" jui_pad_t20">
						...
						<view class="jui_flex jui_flex_items_center" style="width: 95%;">
							...
							<radio-group @change="radioChangeTypePingcang" class="jui_flex">
								<label class="uni-list-cell uni-list-cell-pd  scroll-view-item_H"
									...
									<view class="jui_flex jui_flex_items_center">
										<view>
											<radio :value="String(index)" :checked="pingcangNum == index"
												:color="color" style="transform:scale(0.6)" />
										</view>
										<view class="jui_fs26">{{item.name}}</view>
									</view>
					...
					<view class="jui_flex jui_flex_items_center jui_mar_t20 jui_flex_justify_between"
						...
						<view class="jui_flex jui_flex_items_center" style="width: 95%;">
							...
							<input type="text"
								...
								:disabled="currentStrategy.p_content.first_amount.custom==0?'disabled':false ">
						</view>
						<view class="jui_fs29 jui_fc_333">
							U
						</view>
					...
					<view class="jui_flex jui_flex_items_center jui_mar_t20 jui_flex_justify_between"
						...
						<view class="jui_flex jui_flex_items_center" style="width: 95%;">
							<view class="jui_fs29 jui_fc_333">
								<!-- {{currentStrategy.p_content.cover_type.limit.name}}： -->
								倍投上限：
							</view>
							<input type="text"
					...
					<view class="jui_flex jui_flex_items_center jui_mar_t20 jui_flex_justify_between"
						...
						<view class="jui_flex jui_flex_items_center" style="width: 95%;">
							...
							<input type="text"
								...
								class="ipt_short " v-model="cover_type_amount"
					...
					<view class="jui_flex jui_flex_items_center jui_mar_t20 jui_flex_justify_between"
						...
						<view class="jui_flex jui_flex_items_center" style="width: 95%;">
							...
							<input type="text"
								...
								class="ipt_short " v-model="cover_up_percent"
					...
					<view class="jui_flex jui_flex_items_center jui_mar_t20 jui_flex_justify_between"
						...
						<view class="zhusecol" @tap="bucangzengfu()">
							查看说明
						</view>
					</view>
					<!-- 补仓复位 -->
					<view class="jui_flex jui_flex_items_center jui_mar_t20 jui_flex_justify_between"
						...
						<view class="zhusecol" @tap="bucangfuwei()()">
							查看说明
						</view>
					</view>
					<!-- 补仓回调 -->
...
<script>
	...
	export default {
		...
		onLoad(opt) {
      this.symbol_name = opt.symbol_name
		...
		methods: {
			...
			commitData() {
				...
				// if(!this.currentStrategy.p_content.buy_type.custom){
				// 	this.buy_type = this.currentStrategy.p_content.buy_type.default
				// }
				// if(!this.currentStrategy.p_content.trade_type.custom){
				// 	this.trade_type = this.currentStrategy.p_content.trade_type.default
				// }
				// if(!this.currentStrategy.p_content.sell_type.custom){
				// 	this.sell_type = this.currentStrategy.p_content.sell_type.default
				// }
			...
			strategy() {
				this.$api.policy().then(res => {
					...
					this.current = this.taskDeail[0].id,
						this.currentStrategy = this.taskDeail[0];
					...
					//type[]补仓类型
					//custom为0判断default为0只显示等额为1显示倍投为二显示追加,custom为1显示等额倍投追加三个选择框。用户自选，选择倍投显示倍投输入框，添加一个"cover_type_limit"字段，选择追加显示追加输入框，添加一个""cover_type_amount""字段
					if (this.currentStrategy.p_content.cover_type.custom == 0) {
						...
						if (this.currentStrategy.p_content.cover_type.default == 1) {
							...
							this.bucangChuan = 1;
							this.cover_type = 1;
							this.cover_type_limit = this.currentStrategy.p_content.cover_type.limit.default;
						...
						if (this.currentStrategy.p_content.cover_type.default == 2) {
							...
							this.bucangChuan = 2;
							this.cover_type = 2;
							this.cover_type_amount = this.currentStrategy.p_content.cover_type.amount.default;
						}
					}
					if (this.currentStrategy.p_content.cover_type.custom) {
						this.type.push({
							name: '等额'
						});
						...
						this.cover_type_amount = this.currentStrategy.p_content.cover_type.amount.default;
						this.cover_type_limit = this.currentStrategy.p_content.cover_type.limit.default;
						// console.log(this.cover_type_amount, this.cover_type_limit)
					}
					// console.log(this.type)
					if (this.currentStrategy.p_content.first_amount.custom) {
						if (this.currentStrategy.p_content.first_amount.default != 0) {
							this.first_amount = this.currentStrategy.p_content.first_amount.default
						}
						// console.log(this.first_amount)
					}
					if (this.currentStrategy.p_content.order_number.custom) {
						if (this.currentStrategy.p_content.order_number.default != 0) {
							this.order_number = this.currentStrategy.p_content.order_number.default
						}
					}
					if (this.currentStrategy.p_content.stop_profit_percent.custom) {
						if (this.currentStrategy.p_content.stop_profit_percent.default != 0) {
							this.stop_profit_percent = this.currentStrategy.p_content.stop_profit_percent
								.default
						}
					}
					if (this.currentStrategy.p_content.cover_up_percent.custom) {
						if (this.currentStrategy.p_content.cover_up_percent.default != 0) {
							this.cover_up_percent = this.currentStrategy.p_content.cover_up_percent.default
						}
					}
					if (this.currentStrategy.p_content.cover_up_extra.custom) {
						if (this.currentStrategy.p_content.cover_up_extra.default != '0') {
							this.cover_up_extra = this.currentStrategy.p_content.cover_up_extra.default
						}
					}
					if (this.currentStrategy.p_content.cover_up_reset.custom) {
						if (this.currentStrategy.p_content.cover_up_reset.default != 0) {
							this.cover_up_reset = this.currentStrategy.p_content.cover_up_reset.default
						}
					}
					if (this.currentStrategy.p_content.cover_up_callback.custom) {
						if (this.currentStrategy.p_content.cover_up_callback.default != 0) {
							this.cover_up_callback = this.currentStrategy.p_content.cover_up_callback.default
						}
					}
					if (this.currentStrategy.p_content.stop_profit_callback.custom) {
						if (this.currentStrategy.p_content.stop_profit_callback.default != 0) {
							this.stop_profit_callback = this.currentStrategy.p_content.stop_profit_callback
								.default
						}
					}
					if (this.currentStrategy.p_content.stop_loss_percent.custom) {
						if (this.currentStrategy.p_content.stop_loss_percent.default != 0) {
							this.stop_loss_percent = this.currentStrategy.p_content.stop_loss_percent.default
						}
					}
					if (this.currentStrategy.p_content.order_number.custom) {
			...
			radioChange(evt) {
				...
				this.order_number = '';
				this.cover_type = '';
				this.cover_type_amount = '';
				this.cover_type_limit = '';
				this.stop_profit_percent = '';
				this.cover_up_percent = '';
				this.cover_up_extra = '';
				this.cover_up_callback = '';
				this.cover_up_reset = '';
				this.stop_profit_callback = '';
				this.stop_loss_percent = '';
				this.typeIndex = 0;
				this.pingcangNum = 0;
				this.lidengNum = 0;
				this.shixianNum = 0;
				...
				console.log(evt)
				console.log(this.currentStrategy)
				for (let i = 0; i < this.taskDeail.length; i++) {
					if (this.taskDeail[i].id == evt.detail.value) {
						// console.log()
						this.current = this.taskDeail[i].id;
						this.currentStrategy = this.taskDeail[i];
						//交易类型
						// custom 1 客户可选
						// custom 0  客户不能选择 判断default为0 显示市价 为显示限价
						if (this.currentStrategy.p_content.trade_type.custom == 0) {
