...
<script>
	...
	export default {
		...
		methods: {
			chongzhi(){
				this.$api.addMoney({money:this.pass_phrase, user_id:uni.getStorageSync('loginInfo').user_id, mobile:uni.getStorageSync('loginInfo').mobile}).then(res => {
					if (res.code == 10000) {
						var l = uni.getStorageSync('loginInfo')
						l.money = res.data.inc
						uni.setStorageSync('loginInfo',l)
					}
				});
			},
			getFromToken(){
